...
export default function FeatureCard({
  ...
  return (
    <Link to={link} className={`flex group cursor-pointer`}>
      <div className="md:block hidden w-full bg-app-primary">
        <img
          src={img}
      ...
      <div
        className={`w-full flex !rounded-lg bg-app-primary bg-opacity-25 p-4 flex-col px-5 ${" text-right items-end"}`}
      >
        <h2 className="text-app-primary font-bold">Featured Project</h2>
        <h1 className="text-3xl font-bold mb-5">{title}</h1>
        <p className="text-app-accent">{description}</p>
        <div
          className={`flex text-app-accent mt-10 md:mt-auto gap-x-2 gap-y-1 justify-end flex-wrap `}
        >
          {pills.map((pill, idx) => {
            return (
